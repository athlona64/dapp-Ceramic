...
function RecordSetter() {
  ...
  const updateRecordName = async (name, gender, twitter) => {
    await record.merge({
      name: name,
      gender: gender,
      url: twitter
  ...
  return (
    <div className={styles.content}>
      <div className={styles.mt2}>
        {record.content ? (
          <div className={styles.flexCol}>
            ...
            <span className={styles.subtitle}>Gender: {record.content.gender}!</span>
            <span className={styles.subtitle}>Twitter Link: {record.content.url}!</span>
            <span>
      ...
      <input
        type="text"
        placeholder="Twiitter"
        value={twitter}
        onChange={(e) => setTwitter(e.target.value)}
        className={styles.mt2}
      />            
      <button onClick={() => updateRecordName(name, gender, twitter)}>Update</button>
    </div>
  );
}
